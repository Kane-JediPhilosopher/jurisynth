...
def replace_tables_in_document(results, merged_tables):
    """
    Remove successfully extracted semantic tables from the processed HTML.

    Semantic tables are preserved in the JSON table store and removed
    from the document text.

    Layout containers are recursively linearized into plain text.
    Sparse/formatting tables are removed or converted to bullet text.
    """

    # ---------------------------------------------------------------------
    # Semantic tables
    #
    # A merged semantic table may correspond to multiple physical
    # <table> elements, so remove every tag recorded by merge_fragments().
    # ---------------------------------------------------------------------
...
def extract_only(
    ...
    image_metadata_path = (
        Path(image_store) / f"{doc_id}.json"
    )
