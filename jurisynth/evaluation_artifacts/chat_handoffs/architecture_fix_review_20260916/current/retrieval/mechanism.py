...
class RetrievalMechanism:
    ...
    async def retrieve_evidence(self, request: RetrievalRequest) -> EvidenceBundle:
        ...
        table_hits = _merge_table_hits(table_hits, _tag_table_hits(scoped_table_hits, "source_document"), self.settings.row_top_k)
        image_hits = _merge_image_hits(image_hits, _tag_image_hits(scoped_image_hits, "source_document"), self.settings.table_top_k)
    ...
    def _status_for(self, evidence_items: list, chunk_hits: list[SourceChunk], table_hits: list[TableEvidence]) -> str:
        ...
        if (evidence_items or table_hits) and (strong_structured or strong_table or corroborated_chunk):
            return "success"
...
def _merge_table_hits(
    ...
    for hit in broad + scoped:
        ...
        if (hit.combined_score or 0.0) > (previous.combined_score or 0.0):
            ...
            merged[key] = hit
    return sorted(merged.values(), key=lambda item: item.combined_score or 0.0, reverse=True)[:top_k]
...
def _merge_image_hits(
    ...
    for hit in broad + scoped:
        ...
        if (hit.similarity or 0.0) > (previous.similarity or 0.0):
            hit.retrieval_origins = list(previous.retrieval_origins)
            merged[key] = hit
    return sorted(merged.values(), key=lambda item: item.similarity or 0.0, reverse=True)[:top_k]
