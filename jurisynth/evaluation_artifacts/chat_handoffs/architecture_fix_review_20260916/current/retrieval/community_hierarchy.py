...
def write_orientation_descriptors(
    destination: Path,
    ...
    *,
    ...
    """Persist deterministic, non-authoritative hierarchy orientation metadata.

    Descriptors are graph-only: no chunks, tables, assertions, or generated
    legal conclusions are stored. They support inspection and query-time
    orientation without precomputing LLM leaf summaries.
    """
    if max_anchor_labels < 1:
        raise ValueError("max_anchor_labels must be positive")
    destination.parent.mkdir(parents=True, exist_ok=True)
    nodes = []
    for node in sorted(hierarchy.nodes.values(), key=lambda item: item.community_id):
        anchors = [
            labels.get(member, member.rstrip("/#").rsplit("/", 1)[-1].replace("_", " "))
            for member in node.member_ids[:max_anchor_labels]
        ]
        nodes.append({
    ...
    payload = {
        ...
        "graph_fingerprint": hierarchy.graph_fingerprint,
        "kind": "deterministic_graph_orientation",
        "nodes": nodes,
    }
    destination.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
...
def load_orientation_descriptors(source: Path) -> dict[str, CommunitySummaryInput]:
    """Load graph-only descriptors as non-authoritative lazy-summary inputs.

    The descriptors are deterministic build artifacts.  They intentionally
    contain community shape and anchor labels only—never chunks, tables, or
    assertions—so a later LLM merge cannot become legal evidence.
    """
    payload = json.loads(source.read_text(encoding="utf-8"))
    if (
        not isinstance(payload, dict)
        or payload.get("schema_version") != SCHEMA_VERSION
        or payload.get("descriptor_version") != DESCRIPTOR_VERSION
        or payload.get("kind") != "deterministic_graph_orientation"
        or not isinstance(payload.get("nodes"), list)
    ):
        raise ValueError("Unsupported community orientation descriptor artifact.")
    descriptors: dict[str, CommunitySummaryInput] = {}
    for raw in payload["nodes"]:
        if not isinstance(raw, dict) or not isinstance(raw.get("community_id"), str):
            raise ValueError("Malformed community orientation descriptor.")
        community_id = raw["community_id"]
        if community_id in descriptors:
            raise ValueError(f"Duplicate community orientation descriptor: {community_id}")
        level = raw.get("level")
        member_count = raw.get("member_count")
        child_count = raw.get("child_count")
        anchors = raw.get("anchor_labels")
        if not isinstance(level, int) or not isinstance(member_count, int) or not isinstance(child_count, int):
            raise ValueError("Malformed community orientation descriptor counts.")
        if not isinstance(anchors, list) or not all(isinstance(anchor, str) for anchor in anchors):
            raise ValueError("Malformed community orientation descriptor anchors.")
        detail = f"Community {community_id} (level {level}; {member_count} members; {child_count} children)"
        if anchors:
            detail += "; anchors: " + ", ".join(anchors)
        descriptors[community_id] = CommunitySummaryInput(
            community_id=community_id,
            summary=detail + ".",
            level=level,
            provenance={
...
class CommunityOrientationBuilder:
    hierarchy: CommunityHierarchy
    labels: Mapping[str, str]
    descriptors: Mapping[str, CommunitySummaryInput] | None = None
    max_anchor_labels: int = 6
    ...
    def build(self, community_ids: list[str] | tuple[str, ...]) -> CommunityOrientation | None:
        ...
        for community_id in selected:
            ...
            descriptor = self.descriptors.get(community_id) if self.descriptors is not None else None
