...
@pytest.mark.asyncio
async def test_ordinary_question_gets_source_scoped_image_only_after_primary_retrieval(tmp_path):
    class ChunkIndex:
        def search(self, *_args):
            return [SourceChunk("c1", "doc", "Source-local material", similarity=0.8)]

    mechanism = RetrievalMechanism(
        FakeEmbedder(),
        chunk_indices=[ChunkIndex()],
        image_indices=[_image_index(tmp_path)],
    )
    bundle = await mechanism.retrieve_evidence(RetrievalRequest("q1", "What duties apply to the authority?"))

    assert [item.image_id for item in bundle.image_evidence] == ["doc:image:001"]
    assert bundle.image_evidence[0].retrieval_origins == ["source_document"]
    assert bundle.retrieval_metadata["modality_retrieval"]["image_pass_one_count"] == 0
    assert bundle.retrieval_metadata["modality_retrieval"]["image_pass_two_count"] == 1
