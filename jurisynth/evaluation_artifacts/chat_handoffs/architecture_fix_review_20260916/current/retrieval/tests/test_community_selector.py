...
def test_orientation_expansion_adds_only_shared_lca_without_changing_seed_candidates():
    hierarchy = CommunityHierarchy(
        {
            "left": CommunityNode("left", 0, "root"),
            "right": CommunityNode("right", 0, "root"),
            "root": CommunityNode("root", 1, child_ids=("left", "right")),
        },
        "fixture",
    )
    selector = CommunitySelector(hierarchy=hierarchy)
    selected = [
        type("Candidate", (), {"community_id": "left"})(),
        type("Candidate", (), {"community_id": "right"})(),
    ]

    assert selector.expand_for_orientation(selected) == ("left", "right", "root")
