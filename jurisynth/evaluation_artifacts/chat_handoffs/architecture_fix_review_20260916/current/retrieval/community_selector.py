...
class CommunitySelector:
    ...
    def expand_for_orientation(self, selected: list[CommunityCandidate]) -> tuple[str, ...]:
        """Add at most one shared hierarchy region for non-evidentiary context.

        Candidate communities remain the only retrieval seeds.  The LCA is an
        explanation aid for the Reasoner/UI, not a graph-query constraint, so
        direct evidence cannot be lost through community handling.
        """
        community_ids = tuple(item.community_id for item in selected)
        if self.hierarchy is None or len(community_ids) < 2:
            return community_ids
        lca = self.hierarchy.lca(community_ids)
        return community_ids if lca is None or lca in community_ids else (*community_ids, lca)
