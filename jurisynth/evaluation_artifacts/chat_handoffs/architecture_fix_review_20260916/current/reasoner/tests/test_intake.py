...
from jurisynth.agentic_reasoner.intake import NIMConversationIntake, parse_decision, parse_request_analysis
...
def test_combined_request_analysis_uses_one_structured_call():
    class Model:
        calls = 0

        async def complete(self, **kwargs):
            self.calls += 1
            assert kwargs["response_schema"]["name"] == "request_analysis"
            return '{"action":"proceed","route":"complex","contextual_facts":["Hospital uses an AI device."],"constraints":{"jurisdiction":"EU"},"clarification_question":null}'

    model = Model()
    decision = asyncio.run(NIMConversationIntake(model).decide_and_analyze("Who has which duties?"))

    assert model.calls == 1
    assert decision.route == "complex"
    assert decision.contextual_facts == ("Hospital uses an AI device.",)


def test_combined_request_analysis_validates_route():
    with pytest.raises(ValueError, match="route"):
        parse_request_analysis('{"action":"proceed","route":"maybe","contextual_facts":[],"constraints":{},"clarification_question":null}')
