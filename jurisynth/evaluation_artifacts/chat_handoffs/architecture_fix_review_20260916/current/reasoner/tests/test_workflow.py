...
def test_conflict_explanation_failure_preserves_warnings_and_report():
    ...
    class Explainer:
        async def explain(self, conflicts, answers): raise ValueError("invalid structured output")
    class Log:
        events = []
        def record(self, event, **kwargs): self.events.append(event)
    class Synthesizer:
        async def synthesize(self, query, answers, **kwargs):
            assert kwargs["contradictions"] == [conflict]
            return "report"
    log = Log()
    result = asyncio.run(AgenticWorkflow(
        Analyzer("direct"), AgenticReasoner(Retriever(), generate),
        synthesizer=Synthesizer(), contradiction_detector=Detector(),
        conflict_explainer=Explainer(), reasoning_log=log,
    ).run("question"))
    assert result.report == "report"
    assert result.contradictions == (conflict,)
    assert "contradiction_explanation_failed" in log.events
...
def test_combined_intake_and_analysis_bypasses_second_analyzer_call():
    from jurisynth.agentic_reasoner.intake import RequestAnalysisDecision

    class CombinedIntake:
        calls = 0

        async def decide_and_analyze(self, query, history):
            self.calls += 1
            return RequestAnalysisDecision(
                "proceed",
                "direct",
                ("The request concerns EU law.",),
                {"jurisdiction": "EU"},
            )

    class MustNotRunAnalyzer:
        async def analyze(self, query):
            raise AssertionError("combined intake must be the sole routing call")
    ...
    assert result.analysis.route == "direct"
    assert result.analysis.constraints == {"jurisdiction": "EU"}
