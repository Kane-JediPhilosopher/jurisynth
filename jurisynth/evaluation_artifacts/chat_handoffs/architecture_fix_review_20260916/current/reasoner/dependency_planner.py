...
from jurisynth.agentic_reasoner.schemas import DEPENDENCY_SCHEMA
...
class SemanticDependencyPlanner:
    ...
    async def plan(self, leaves: tuple[LeafNode, ...]) -> tuple[LeafNode, ...]:
        ...
        response = await self.model.complete(system=_SYSTEM_PROMPT, user=json.dumps(payload), max_tokens=self.max_tokens, response_schema=DEPENDENCY_SCHEMA)
